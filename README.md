...
| 第二步 DCF | 自由现金流折现三情景估值（破产清算/保守/中性）+ 永续增长率×WACC 敏感性网格；合理估值上限用「5年PE中位数×当前EPS」封顶 |
...
- **自由现金流**：FCF = 经营现金流净额 − 资本性支出 × 0.7（仅 70% capex 视为维持性支出）。
- **折现率 WACC**：固定 9.5%（不再随情景变化）。
- **三情景**：保守（0% 永续）/ 中性（1.5% 永续）/ 破产清算（0 增长且折旧摊销不计入）。显性 5 年增长率 = 永续增长率（成熟股无独立高增长期）。
- **破产清算**：现金流 = FCF − 折旧摊销（移除非现金加回项），为估值底值；D&A 不可得时回退归母净利润，并对 `liquidation ≤ 保守` 钳位以保单调。
- **合理估值上限**：`min(中性 DCF, 过去 5 年 PE 中位数 × 当前 EPS)`，作为估值天花板，避免 DCF 对成熟股过度外推。EPS = 最新归母净利润 / 总股本；5 年 PE 中位数取自个股历史 PE。
...
- **破产清算 D&A 取数**：折旧摊销从现金流量表取（合并列优先，否则折旧+摊销分列求和）。sina 现金流量表若以行（非列）返回 D&A 导致提取为空，破产清算自动回退归母净利润口径并对 `破产清算 ≤ 保守` 钳位保单调，属可接受降级。离线 demo 已含 D&A 字段验证主路径。

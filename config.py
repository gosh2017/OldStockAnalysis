...
HIKYUU_INDUSTRY_TO_BUCKET: dict[str, str] = {
    ...
    # ---- 新浪行业板名（批量筛选行业 fallback 用；与申万/东财并行）----
    # 新浪行业分类无「银行/证券」细分，统归「金融行业」→ 非银金融（is_financial
    # 口径：跳过资产负债率/OCF 打分，与银行/非银同源；非「其他」会错把 ~90% 负债记分）。
    "金融行业": "非银金融",
    "采掘行业": "周期", "供水供气": "周期", "陶瓷行业": "周期",
    "船舶制造": "成长", "飞机制造": "成长",
    "家具行业": "消费", "物资外贸": "消费", "开发区": "周期",
}
...
# -- Hikyuu 本地库路径与财报字段（单股历史数据迁移后用）--------------
# stock.db 实测位于 G:/QTrading/StockData（~/.hikyuu/importdata-gui.ini 的 [hdf5] dir），
# 非 c:\stock。用于直读 zh_bond10（国债历史）与 finance 自省；HDF5 kdata 经 sm 查询，不直读此路径。
HIKYUU_DB_PATH = r"G:/QTrading/StockData/stock.db"

# Hikyuu HistoryFinance 三表字段名（按名取，避免 id off-by-one：DB id N 在数组下标 N-1，
# 运行时 get_history_finance_field_index(name) 返回 id-1 的 0 基数组下标）。
# 金额字段经量级核实为「元」（非万元）：归母净利 ~4e10、权益 ~4.5e11、OCF/capex 同量级
# （000001 平安银行 2025Q3 探针）。% 字段（ROE/资产负债率）为 0-100 尺度；总股本字段为「股」
# （与 weight.total_count 万股不同——后者须 ×1e4）；每股净资产为「元/股」。
HKYUU_FINANCE_FIELDS = {
    "归母净利润": "利润表_归属于母公司所有者的净利润",   # DB id 96
    "净利润":     "利润表_净利润",                       # DB id 95（兜底）
    "经营现金流": "现金流量表_经营活动产生的现金流量净额",  # DB id 107
    "capex":      "现金流量表_购建固定资产、无形资产和其他长期资产支付的现金",  # id 114
    "折旧":       "现金流量表_固定资产折旧、油气资产折耗、生产性生物资产折旧",  # id 136
    "摊销":       "现金流量表_无形资产摊销",             # DB id 137
    "ROE_加权":   "盈利能力_加权净资产收益率(每股指标)",   # DB id 281
    "资产负债率": "资本结构_资产负债率",                 # DB id 210
    "归母权益":   "资产负债表_归属于母公司股东权益",     # DB id 271
    "每股净资产": "每股净资产",                         # DB id 4
    "总股本":     "股本股东_总股本",                     # DB id 238
}
